...
sincwin_t *sincwin_create(uint32_t halflen, uint32_t num_phases, float freq, float (*window_fn)(float pos, void *user), void *user)
{
  ...
  *t++ = 0.0f; // used as last coeff in phase=0, or when rounded to phase=1
...
void sincwin_calc_fir(sincwin_t *sw, float *dst, float phase)
{
  ...
  const uint32_t p = (uint32_t)(phase * num_phases + 0.5f);

  // TODO? larger table could get rid of conditional...?
  const float *s = sw->table + (p == num_phases ? (num_phases - 1) * halflen + 1 : (num_phases - 1 - p) * halflen);
  ...
  s = sw->table + (p == 0 ? (num_phases - 1) * halflen + 1 : (p - 1) * halflen);
  d = dst + halflen;
  for (uint32_t i = 0; i < halflen; i++) {
    *d = *s;
    ++s;
